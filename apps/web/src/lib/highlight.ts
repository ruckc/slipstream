...
const SUPPORTED_LANGS = [
  'typescript', 'javascript', 'tsx', 'jsx', 'html', 'css', 'scss', 'less',
  'json', 'yaml', 'toml', 'markdown', 'mdx', 'svelte', 'vue',
  'python', 'rust', 'go', 'java', 'kotlin', 'swift', 'c', 'cpp', 'csharp',
  'bash', 'sh', 'zsh', 'fish', 'powershell', 'dockerfile',
  'sql', 'graphql', 'proto', 'xml',
  'ruby', 'php', 'lua', 'r', 'scala',
  'nginx', 'ini', 'env', 'diff', 'git-commit', 'text',
] as const
...
export function isCodeFile(filename: string): boolean {
  ...
  const codeExts = new Set([
    ...
    'dockerfile', 'sql', 'graphql', 'gql', 'proto', 'xml',
    'rb', 'php', 'lua', 'r', 'scala',
    'nginx', 'ini', 'env', 'diff', 'txt',
  ])
  const baseName = filename.split('/').pop()?.toLowerCase() ?? ''
