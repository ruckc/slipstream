...
export async function getProjectPermissions(
  ...
  return db.select().from(projectPermissions).where(eq(projectPermissions.projectId, projectId))
}
...
export async function setProjectPermissions(
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.delete(projectPermissions).where(eq(projectPermissions.projectId, projectId))
