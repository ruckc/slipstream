...
The Rust agent reads: `PORT`, `JWKS_URL` (required; must be http/https URL), `PROJECT_ID`, `WORKSPACE_PATH`, `HOME_PATH`, `IDLE_TIMEOUT_SECONDS`, `CORS_ORIGIN` (required — agent exits at startup if unset), `METRICS_TOKEN` (optional; if set, `/metrics` requires `Authorization: Bearer <token>`; if unset, `/metrics` returns 403). Registry-related env injected into pods by the controller (only when the namespace has registry credentials): `REGISTRY_HOST` and `REGISTRY_INSECURE` (dev). buildx is wired to the always-present buildkit sidecar via `BUILDX_BUILDER=slipstream` + `/etc/profile.d/buildx.sh` baked into the agent image (no `DOCKER_HOST`).
