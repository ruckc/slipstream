...
Container images are built inside project pods using the **Docker CE CLI + buildx plugin** (`docker-ce-cli` + `docker-buildx-plugin`, installed from Docker's official apt repo — not the Debian `docker.io` package) talking to a **moby/buildkit rootless sidecar** container. The sidecar is injected into **every** project pod (building must not require a registry), runs BuildKit in rootless mode (`--oci-worker-no-process-sandbox`, with both seccomp **and AppArmor** unconfined so the rootless worker's unshare/mount syscalls aren't blocked on AppArmor nodes) and exposes a standalone `buildkitd` over a Unix socket at `/var/run/buildkit/buildkitd.sock`, shared with the agent container via an emptyDir volume. It carries no readiness probe so a buildkit hiccup can't drop the pod from Service endpoints and take the agent offline. Because that socket is **buildkitd, not a Docker daemon**, `DOCKER_HOST` is left unset; instead the agent image registers a buildx **`remote`-driver builder** pointing at the socket (via `/etc/profile.d/buildx.sh`, recreated on login since `/home/agent` is a PVC) and selects it with `BUILDX_BUILDER=slipstream`, so `docker build` / `docker buildx build --push` work without any daemon in the agent itself. The registry is always accessed via its public FQDN (`harbor.ruck.io`) which has a valid TLS certificate — no insecure configuration is needed. Typical flow in a pod terminal: `docker buildx build -t $REGISTRY_HOST/<namespace>/<project>/<img>:<tag> --push /workspace`.
...
Each Slipstream namespace maps to one **private Harbor project** (named after the slug). The **project-controller** owns all Harbor provisioning, in its reconcile loop (`ensureHarborRegistry`, `internal/controller/harbor.go`): using the Harbor admin API it ensures the private project plus two project-scoped **robot accounts** (push+pull, and pull-only) exist. Because Harbor returns a robot secret only at creation, the controller persists the robots in a per-namespace Secret (`harbor-creds-<nsslug>`) in its **own** namespace, which doubles as a provisioning lock so sibling projects in the same namespace don't double-provision. This is level-triggered: any project — including those created before the registry was configured — gets credentials on the next reconcile, with no web/UI action. Harbor itself enforces isolation — a namespace's robot can only access its own private project. (The web app no longer writes to Harbor or the database for registry provisioning; it only *reads* Harbor for the registry-browse UI via the admin API in `src/lib/server/registry/harbor.ts`.)

Credential delivery to pods: the controller materializes the robot credentials as `kubernetes.io/dockerconfigjson` Secrets — `slipstream-registry-auth` (push+pull) in the project namespace, mounted read-only at `/etc/registry-auth` and at `/home/user/.docker` in the buildkit sidecar, and `slipstream-registry-pull-auth` (pull-only) in the workspace namespace. The Secret volume **remaps the `.dockerconfigjson` key to `config.json`** so BuildKit (`$HOME/.docker/config.json`) actually finds it — buildkitd authenticates pulls of base images and pushes of results to the namespace's private Harbor project without an interactive login. The pod NetworkPolicy is extended with egress to the Harbor namespace. The robot secret never travels through the cluster-scoped CR spec anymore.
...
The Rust agent reads: `PORT`, `JWKS_URL` (required; must be http/https URL), `PROJECT_ID`, `WORKSPACE_PATH`, `HOME_PATH`, `IDLE_TIMEOUT_SECONDS`, `CORS_ORIGIN` (required — agent exits at startup if unset), `METRICS_TOKEN` (optional; if set, `/metrics` requires `Authorization: Bearer <token>`; if unset, `/metrics` returns 403). Registry-related env injected into pods by the controller (only when the registry is enabled): `REGISTRY_HOST` (the registry host, public FQDN) and `REGISTRY` (the full `host/namespace/project` image-ref prefix, so `docker build -t $REGISTRY/<repo>:<tag>` works) and `REGISTRY_INSECURE` (dev). buildx is wired to the always-present buildkit sidecar via `BUILDX_BUILDER=slipstream` + `/etc/profile.d/buildx.sh` baked into the agent image (no `DOCKER_HOST`).

The project-controller additionally reads: `USAGE_REPORT_URL` (optional, for usage telemetry), `KUBECONFIG` (optional, falls back to in-cluster config), `HARBOR_NAMESPACE` (grants pods egress to Harbor), `REGISTRY_INSECURE` (`true` in dev so docker buildx treats REGISTRY_HOST as insecure), `BUILDKIT_IMAGE` (defaults to pinned moby/buildkit digest), and — for Harbor provisioning — `HARBOR_URL`, `HARBOR_ADMIN_USERNAME`, `HARBOR_ADMIN_PASSWORD`, `REGISTRY_HOST` (registry provisioning activates when `HARBOR_URL` + `REGISTRY_HOST` are set).
