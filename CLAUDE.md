...
Container images are built inside project pods using the **Docker CE CLI + buildx plugin** (`docker-ce-cli` + `docker-buildx-plugin`, installed from Docker's official apt repo — not the Debian `docker.io` package) talking to a **moby/buildkit rootless sidecar** container. The sidecar runs BuildKit in rootless mode (`--oci-worker-no-process-sandbox`) and exposes a standalone `buildkitd` over a Unix socket at `/var/run/buildkit/buildkitd.sock`, shared with the agent container via an emptyDir volume. Because that socket is **buildkitd, not a Docker daemon**, `DOCKER_HOST` is left unset; instead the agent image registers a buildx **`remote`-driver builder** pointing at the socket (via `/etc/profile.d/buildx.sh`, recreated on login since `/home/agent` is a PVC) and selects it with `BUILDX_BUILDER=slipstream`, so `docker build` / `docker buildx build --push` work without any daemon in the agent itself. The registry is always accessed via its public FQDN (`harbor.ruck.io`) which has a valid TLS certificate — no insecure configuration is needed. Typical flow in a pod terminal: `docker buildx build -t $REGISTRY_HOST/<namespace>/<project>/<img>:<tag> --push /workspace`.
